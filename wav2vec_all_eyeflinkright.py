...
def process_folder(folder_path, model_name="facebook/wav2vec2-base-960h"):
    ...
    for i in range(len(wav_files)):
        ...
        with open(os.path.join(folder_path, json_files[i]), 'r') as file:
            ...
            if "blendshapes" in data:
                ...
                if "eyeBlinkRight" in data["blendshapes"]:
                    # Append the "jawOpen" values to the list

                    eyeBlinkRight_list.extend(data["blendshapes"]["eyeBlinkRight"])
                    n = len(data["blendshapes"]["eyeBlinkRight"])
        # 使用切片进行降采样
    ...
    return all_hidden_states,eyeBlinkRight_list
...
hidden_states_list,eyeBlinkRight = process_folder(path)
...
print(len(eyeBlinkRight))

eyeBlinkRight_tensor = torch.tensor(eyeBlinkRight, dtype=torch.float32)


# 重塑 x 和 y 的形状以适应线性回归
x_reshaped = normalized_hidden_states_list.reshape(-1, 768)
y_reshaped = eyeBlinkRight_tensor.reshape(-1, 1)
